...
library(knitr)
```
With the exception of `googlesheets4`, `ggtext`, and `knitr`, these packages are all a part of the tidyverse, and could have been called with one line, `library(tidyverse)`, but I thought it'd be useful to call each one explicitly to explain its use.  
...
I could have saved a bit of code by writing a new theme file, but I was being a bit lazy. There's a general rule of thumb, *"anytime you copy and paste code three times, you should just think about writing a function.* I think I copied and pasted the theme for just about every chart. 

If you read this, I hope it ended up being helpful. This is a fun pasttime for me, so I'm happy to answer any questions you might have, just [shoot me an email](mailto:mark.rieke@siemens-energy.com).
